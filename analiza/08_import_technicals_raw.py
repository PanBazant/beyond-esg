...
TEMPLATE_PATH = INPUT_DIR / "company_technicals_template.csv"
TARGET_PATH = INPUT_DIR / "company_technicals.csv"
REPORT_PATH = OUT_DIR / "company_technicals_import_report.json"
UNMATCHED_PATH = OUT_DIR / "company_technicals_unmatched.csv"
...
HEADER_ALIASES = {
    "symbol": {"symbol", "ticker", "ticker_symbol", "instrument", "code", "stock"},
    "company_name": {"company", "companyname", "name", "issuer", "securityname"},
    "momentum_30d": {"momentum30d", "change1m", "performance1m", "return1m", "pricechange30d"},
    "momentum_90d": {"momentum90d", "change3m", "performance3m", "return3m", "pricechange90d"},
    "volatility_30d": {"volatility30d", "vol30d", "stddev30d", "historicalvolatility30d"},
    "drawdown_90d": {"drawdown90d", "maxdrawdown90d", "drawdown3m", "maxdrawdown3m"},
    "source": {"source", "provider", "vendor", "datasource"},
    "as_of_date": {"asofdate", "date", "snapshotdate", "pricedate"},
}
...
def load_base_rows() -> tuple[list[dict], dict[str, dict]]:
    ...
    if not source_path.exists():
        raise FileNotFoundError(
            "Brakuje bazowego pliku company_technicals_template.csv. "
            "Najpierw uruchom 07_build_technical_features.py, aby go wygenerowac."
        )
...
def detect_mapping(headers: list[str]) -> dict[str, str]:
    ...
    if "symbol" not in mapping:
        raise ValueError("Nie udalo sie wykryc kolumny symbol/ticker w surowym eksporcie technicznym.")
...
def main() -> None:
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"Brak pliku z surowym eksportem technicznym: {input_path}")
    ...
    for raw_row in raw_rows:
        ...
        for metric_field in ("momentum_30d", "momentum_90d", "volatility_30d", "drawdown_90d"):
            source_header = mapping.get(metric_field)
            if not source_header:
                continue

            parsed = parse_metric(raw_row.get(source_header))
