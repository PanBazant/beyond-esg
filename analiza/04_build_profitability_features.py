...
SUMMARY_PATH = OUT_DIR / "company_profitability_features_summary.json"
PROFITABILITY_FEATURES_SAMPLE_PATH = OUT_DIR / "company_profitability_features_sample.jsonl"
SUMMARY_SAMPLE_PATH = OUT_DIR / "company_profitability_features_sample_summary.json"
...
def normalize_value(value: float | None, bounds: tuple[float, float] | None) -> float | None:
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for index, raw_row in enumerate(reader, start=1):
            ...
            for metric_name in METRIC_WEIGHTS:
                row[metric_name] = parse_ratio(raw_row.get(metric_name))
            rows.append(row)
...
def main() -> None:
    ...
    parser.add_argument("--input-file", type=str, default=str(FUNDAMENTALS_INPUT_PATH))
    ...
    output_path = PROFITABILITY_FEATURES_SAMPLE_PATH if args.sample else PROFITABILITY_FEATURES_PATH
    summary_path = SUMMARY_SAMPLE_PATH if args.sample else SUMMARY_PATH
    companies = load_companies()

    if not input_path.exists():
        export_template(FUNDAMENTALS_TEMPLATE_PATH, companies)
        summary = {
            "input_file": str(input_path),
            "template_file": str(FUNDAMENTALS_TEMPLATE_PATH),
            "rows_loaded": 0,
            "rows_written": 0,
            "message": "Brak pliku z fundamentami. Wygenerowano szablon do uzupelnienia.",
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        for row in rows:
            ...
            for metric_name, weight in METRIC_WEIGHTS.items():
                normalized = normalize_value(row.get(metric_name), metric_bounds[metric_name])
            ...
            if available_weight > 0:
                profitability_score = round(100 * (weighted_score / available_weight), 2)
            ...
            feature_row = {
                ...
                "profitability_completeness": round(available_weight, 4),
                "profitability_metric_count": len(normalized_metrics),
                "profitability_score": profitability_score,
                "profitability_metric_version": "fundamentals-v1-manual",
            }
